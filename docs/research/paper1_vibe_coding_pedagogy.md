...
**以物聯網前端開發課程為例的教學框架設計研究**

*Vibe Coding Pedagogy: A Teaching Framework Design Study Based on an IoT
...
integration）與知識展示（artifact creation）等核心要素。以 ESP32
遙控車控制介面為核心的課程設計，提供了高度真實的工程情境：學習者必須整合
Web
前端技術、行動端觸控互動、資料封裝（JSON/TypedArray）以及藍牙低功耗（BLE）通訊，才能讓真實的硬體裝置正常運作。這種以真實硬體作為驗收標準的設計，使學習者的學習動機與問題解決的真實性大幅提升，也使
Vibe Coding 不再是為了練習而練習，而是為了解決真實的工程問題。
...
課程以 Web 技術（HTML5、CSS Flexbox/Grid、JavaScript）結合 ESP32
藍牙低功耗（BLE）硬體為情境，引導學習者開發遙控車操控介面。透過 Vibe
Classroom 平台，以 GitHub Actions 自動評分機制在每次 git push
後自動執行測試腳本，提供即時的功能正確性回饋。課程共設計超過十個獨立模組，涵蓋從基礎開發環境設置（VS
Code、GitHub 身份設定）到進階硬體整合（Web Bluetooth API、TypedArray
二進制資料封裝）的完整學習路徑。
...
失效安全機制）。這一階段以遙控車控制面板為情境，使學習者在真實的 UX
問題（如 iOS 300ms 點擊延遲、touchstart 與 mousedown 的衝突、Fitts\' Law
觸控熱區設計）中學習技術概念，而非在脫離情境的語法練習中。

CSS Grid 3×3
十字鍵模組（car-starter-control-panel）是這一階段的代表性設計。學習者需實作一個在手機直向（Portrait）與橫向（Landscape）模式下均不變形的十字鍵控制面板，具備引擎啟動/關閉的動態禁用狀態、物理按壓下沉感（translateY +
inset shadow）以及多層次 z-index HUD
疊加效果。這些設計挑戰在工業界的行動端 Web 開發中具有高度的真實性。

**3. 第三階段：資料處理與硬體通訊**

涵蓋 JSON 封包設計與優化（Packet Slimming：將 46 位元組壓縮至 15
位元組，減量達 68%）、TypedArray 與 DataView
二進制資料封裝（解決位元組序問題與精準二進制格式）、HTML5 Canvas
圖形渲染（虛擬搖桿的座標系統、Math.atan2 方向計算、requestAnimationFrame
動畫循環），以及 Web Bluetooth API 的 BLE 裝置搜尋、配對授權與 GATT
Characteristic 讀寫。

這一階段涉及最廣泛的跨領域知識整合，也最能體現 Vibe Coding
的教育優勢------學習者可以透過 AI 快速理解 GATT 協定、Characteristic
UUID、ArrayBuffer
等陌生概念，再將精力集中在關鍵細節的深度理解與實機驗證上。Web Bluetooth
API 目前僅在 Chrome（桌機版）上完整支援，iOS Safari
完全不支援，這一跨平台相容性問題迫使學習者深入理解瀏覽器標準化的局限，是一個真實工程師每天都在面對的問題。
...
第一，以真實問題為驅動的學習設計顯著提升了任務的意義感。學習者在開發 BLE
遙控介面時，需要整合觸控事件、JSON
封包設計、非同步程式設計（async/await、BLE
狀態機）與硬體通訊等多個知識領域；這種複雜度在傳統課程中可能造成認知超載，但在
Vibe Coding 輔助下，學習者可以先透過 AI
...
課程涵蓋的技術棧廣度------Web Bluetooth API、Canvas API、TypedArray、CSS
Grid、Touch Events------加上 ESP32
韌體的硬體層面，對學習者和教學者均構成相當的認知負荷。AUDIT.md
顯示，學習者在 BLE 跨平台相容性（iOS 不支援 Web Bluetooth
API）的問題上投入大量調試時間，這雖然是真實工程師每天面對的問題，但對初學者可能造成挫折感，進而降低學習動機。如何在真實複雜度與學習可行性之間取得平衡，是這類課程持續面對的核心挑戰。
...
本研究以「Vibe Coding 教學實驗室」課程為案例，提出 PACE 教學框架（Prompt
...
程式碼審計機制（AUDIT.md）、帶有問責標記的微提交策略，以及兼顧功能正確性與過程理解的多層評量設計。此外，漸進複雜度的四階段任務序列設計、以真實硬體作為驗收標準，以及
GitHub Actions 自動化回饋迴路，也是有效課程設計的重要元素。

第三，回應 RQ3（相關教育理論的適用性）：建構主義（AI 作為
MKO）、鷹架理論（漸進式 AI 支援退出）與
PBL（真實物聯網情境作為驅動性問題）三個理論框架，均能有效為 PACE
框架提供理論依據，且三者之間具有良好的互補性。

本研究的理論貢獻在於為 Vibe Coding
教學提供了一個兼具理論根據（建構主義、鷹架理論、PBL）與實踐操作性（PACE
四要素）的課程設計框架，填補了現有研究中以教育工作者為中心（Munoz &
Rook, 2026）或以工作流分析為主（Sarkar & Drosos,
2025）的研究之間，缺乏以學習者能力培育為核心的課程設計框架這一空白。
