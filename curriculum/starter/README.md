...
│   ├── car-starter-html5-basics-kUUp1Pe8 / car-starter-html5-basics-ChenYuiLiang
│   ├── car-starter-flexbox-layout-kUUp1Pe8
│   └── car-starter-ui-ux-standards-kUUp1Pe8
├── 2. start-02-master-web-ble (Web Bluetooth 藍牙通訊)
│   ├── car-starter-ble-async-kUUp1Pe8
│   ├── car-starter-ble-security-kUUp1Pe8
│   └── car-starter-typed-arrays-kUUp1Pe8
├── 3. start-03-master-remote-control (遙控面板與資料傳輸)
│   ├── car-starter-control-panel-kUUp1Pe8
│   ├── car-starter-data-json-kUUp1Pe8
│   └── car-starter-flow-logic-kUUp1Pe8
├── 4. start-04-master-touch-events (行動端觸控與手勢控制)
│   ├── car-starter-touch-basics-kUUp1Pe8
│   ├── car-starter-touch-vs-mouse-kUUp1Pe8
│   ├── car-starter-prevent-default-kUUp1Pe8
│   └── car-starter-long-press-kUUp1Pe8
└── 5. start-05-master-joystick-lab (Canvas 虛擬搖桿實驗室)
    ├── car-starter-canvas-joystick-kUUp1Pe8
    └── car-starter-joystick-math-kUUp1Pe8
```
...
### 1-1 HTML5 語義化標籤與滿版容器 (`car-starter-html5-basics-kUUp1Pe8`)
...
### 1-2 CSS Flexbox 遙控器排版 (`car-starter-flexbox-layout-kUUp1Pe8`)
...
### 1-3 UI/UX 行動端設計規範 (`car-starter-ui-ux-standards-kUUp1Pe8`)
...
### 2-1 Web Bluetooth API 異步連線 (`car-starter-ble-async-kUUp1Pe8`)
...
### 2-2 BLE 配對與安全機制 (`car-starter-ble-security-kUUp1Pe8`)
- **核心觀念**：處理藍牙連線斷開 `gattserverdisconnected` 事件，實作自動重連機制。

### 2-3 TypedArrays 二進位指令封包 (`car-starter-typed-arrays-kUUp1Pe8`)
...
### 3-1 遙控控制台 UI (`car-starter-control-panel-kUUp1Pe8`)
- 整合表單元件 `<input type="range">` 設定速度，`<progress>` 顯示實時輸出，`<ul>` 輸出系統日誌。

### 3-2 JSON 控制指令序列化 (`car-starter-data-json-kUUp1Pe8`)
- 使用 `JSON.stringify({ cmd: 'drive', v: 80, w: 30 })` 封裝遙控傳送指令。

### 3-3 控制流程與狀態維護 (`car-starter-flow-logic-kUUp1Pe8`)
...
### 4-1 Touch Events 處理 (`car-starter-touch-basics-kUUp1Pe8`)
- 監聽 `touchstart` / `touchmove` / `touchend` 獲取 `e.touches[0].clientX/clientY`。

### 4-2 Touch vs Mouse 雙相相容 (`car-starter-touch-vs-mouse-kUUp1Pe8`)
- 封裝通用 pointer 事件，確保在電腦瀏覽器與手機觸控板均可順暢操作。

### 4-3 防止預設縮放與手勢 (`car-starter-prevent-default-kUUp1Pe8`)
- 在觸控區域調用 `e.preventDefault()` 徹底關閉雙擊放大與 Safari 拉動下拉刷新手勢。

### 4-4 長按與手勢發送 (`car-starter-long-press-kUUp1Pe8`)
...
### 5-1 Canvas 繪製虛擬搖桿 (`car-starter-canvas-joystick-kUUp1Pe8`)
- 利用 2D Canvas Context 繪製搖桿外圈底盤與內圈滑塊。

### 5-2 搖桿極座標與向量計算 (`car-starter-joystick-math-kUUp1Pe8`)
